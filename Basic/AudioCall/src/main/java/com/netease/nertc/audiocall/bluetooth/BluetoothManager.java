/*
 * Copyright (c) 2014-2022 NetEase, Inc.
 * All right reserved.
 */
...
import com.netease.nertc.audiocall.utils.BluetoothUtil;
...
public class BluetoothManager extends DemoBluetoothManager {
    ...
    public BluetoothManager(Context context, DemoAudioManager manager, int bluetoothSCOTimeoutMs) {
    ...
    public void start() {
        ...
        if (!mBluetoothAdapter.getProfileProxy(mContext, mBluetoothServiceListener, BluetoothProfile.HEADSET)) {
    ...
    private class BluetoothServiceListener implements BluetoothProfile.ServiceListener {
        ...
        public void onServiceConnected(int profile, BluetoothProfile proxy) {
            ...
            Logging.i(TAG, "BluetoothServiceListener.onServiceConnected: bluetooth state=" + mBluetoothState);
            mBluetoothHeadset = (BluetoothHeadset) proxy;
            mBluetoothState = State.HEADSET_AVAILABLE;
            updateAudioDeviceState();
            Logging.i(TAG, "BluetoothServiceListener.onServiceConnected done: bluetooth state=" + mBluetoothState);
        }
        ...
        public void onServiceDisconnected(int profile) {
            ...
            Logging.i(TAG, "BluetoothServiceListener.onServiceDisconnected: bluetooth state=" + mBluetoothState);
            stopScoAudio();
            mBluetoothHeadset = null;
            mBluetoothDevice = null;
            mBluetoothState = State.HEADSET_UNAVAILABLE;
            updateAudioDeviceState();
            Logging.i(TAG, "BluetoothServiceListener.onServiceDisconnected done: bluetooth state=" + mBluetoothState);
        }
    }
    ...
    private class BluetoothHeadsetBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothHeadset.ACTION_CONNECTION_STATE_CHANGED.equals(action)) {
                ...
                final int preState = intent.getIntExtra(BluetoothHeadset.EXTRA_PREVIOUS_STATE, BluetoothHeadset.STATE_DISCONNECTED);
                final BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);

                Logging.i(TAG, "BluetoothHeadsetReceiver.onReceive: "
                        + "a=ACTION_CONNECTION_STATE_CHANGED, "
                        + "s=" + connectionStateToString(state) + ", "
                        + "ps=" + connectionStateToString(preState) + ", "
                        + "sb=" + isInitialStickyBroadcast() + ", "
                        + "bluetooth state=" + mBluetoothState + ", "
                        + "d=" + (device == null ? "null" : device.getName()) + ", "
                        + "ds=" + connectionStateToString(mBluetoothHeadset == null ? -1 : mBluetoothHeadset.getConnectionState(device)));
                ...
                final int preState = intent.getIntExtra(BluetoothHeadset.EXTRA_PREVIOUS_STATE, BluetoothHeadset.STATE_AUDIO_DISCONNECTED);
                final BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);

                Logging.i(TAG, "BluetoothHeadsetReceiver.onReceive: "
                        + "a=ACTION_AUDIO_STATE_CHANGED, "
                        + "s=" + audioStateToString(state) + ", "
                        + "ps=" + audioStateToString(preState) + ", "
                        + "sb=" + isInitialStickyBroadcast() + ", "
                        + "bluetooth state=" + mBluetoothState + ", "
                        + "d=" + (device == null ? "null" : device.getName()) + ", "
                        + "d_type =" + BluetoothUtil.getBluetoothDeviceType(device) + ", "
                        + "sco=" + (mBluetoothHeadset == null ? "false" : mBluetoothHeadset.isAudioConnected(device)));
                ...
                if (state == BluetoothHeadset.STATE_AUDIO_CONNECTED) {
                    ...
                    if (mBluetoothState == State.SCO_CONNECTING) {
                        ...
                        Logging.e(TAG, "BluetoothHeadsetReceiver.Unexpected state BluetoothHeadset.STATE_AUDIO_CONNECTED");
                    }
                } else if (state == BluetoothHeadset.STATE_AUDIO_DISCONNECTED) {
                    Logging.i(TAG, "BluetoothHeadsetReceiver.bluetooth audio sco is now disconnected");
                    if (isInitialStickyBroadcast()) {
                        Logging.i(TAG, "BluetoothHeadsetReceiver.Ignore STATE_AUDIO_DISCONNECTED initial sticky broadcast.");
                        return;
                    }
                    updateAudioDeviceState();
                }
            } else if (action.equals(AudioManager.ACTION_SCO_AUDIO_STATE_UPDATED)) {
                final int state = intent.getIntExtra(AudioManager.EXTRA_SCO_AUDIO_STATE, AudioManager.SCO_AUDIO_STATE_DISCONNECTED);
                final int preState = intent.getIntExtra(AudioManager.EXTRA_SCO_AUDIO_PREVIOUS_STATE, AudioManager.SCO_AUDIO_STATE_DISCONNECTED);
                Logging.i(TAG, "BluetoothHeadsetReceiver.onReceive: "
                        + "a=ACTION_SCO_AUDIO_STATE_UPDATED, "
                        + "s=" + scoStateToString(state) + ", "
                        + "ps=" + scoStateToString(preState) + ", "
                        + "sb=" + isInitialStickyBroadcast() + ", "
                        + "bluetooth state=" + mBluetoothState);
            }
        }
    ...
    public boolean startScoAudio() {
        Logging.i(TAG, "startScoAudio: bluetooth state=" + mBluetoothState
                + ", attempts: " + mScoConnectionAttempts
                + ", sco is on: " + mAudioManager.isBluetoothScoOn());
        ...
        Logging.i(TAG, "startScoAudio done: bluetooth state = " + mBluetoothState + " , sco is on: " + mBlueToothSCO);
        return true;
    }
    ...
    public void stopScoAudio() {
        Logging.i(TAG, "stopScoAudio: bluetooth state=" + mBluetoothState + ", sco is on: " + mAudioManager.isBluetoothScoOn() + " , needStopSco:" + needStopSco);
    ...
    private void logBluetoothAdapterInfo(BluetoothAdapter localAdapter) {
        try {
            Logging.i(TAG, "BluetoothAdapter: "
    ...
    private void bluetoothTimeout() {
        ...
        Logging.e(TAG, "bluetoothTimeout: bluetooth state=" + mBluetoothState + ", "
                + "attempts: " + mScoConnectionAttempts + ", "
                + "sco is on: " + mAudioManager.isBluetoothScoOn());
        if (mBluetoothState != State.SCO_CONNECTING) {
            return;
        ...
        if (!devices.isEmpty()) {
            ...
            if (mBluetoothHeadset.isAudioConnected(mBluetoothDevice)) {
                Logging.i(TAG, "sco connected with " + mBluetoothDevice.getName() + " , type: " + BluetoothUtil.getBluetoothDeviceType(mBluetoothDevice));
                scoConnected = true;
            } else {
                if (!mBlueToothSCO ){
    ...
    public void updateDevice() {
        ...
        if (devices.isEmpty()) {
            ...
            for (BluetoothDevice device : devices) {
                ...
                Logging.i(TAG, "updateDevice connected bluetooth headset: "
                        + ", name=" + device.getName() + ", "
                        + ", state=" + connectionStateToString(mBluetoothHeadset.getConnectionState(device))
                        + ", type=" + BluetoothUtil.getBluetoothDeviceType(device)
                        + ", sco audio=" + mBluetoothHeadset.isAudioConnected(device));
