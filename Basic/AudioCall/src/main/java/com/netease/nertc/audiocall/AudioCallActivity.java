...
public class AudioCallActivity extends AppCompatActivity implements NERtcCallback, View.OnClickListener {
    ...
    private DemoAudioManager mDemoAudioManager;
    ...
    private AppCompatSpinner mAudioRouteSp;
    ...
    private void initAudioManager() {
        ...
        NERtcParameters.Key disableAudioRoutekey = NERtcParameters.Key.createSpecializedKey("key_disable_sdk_audio_route");
        parameters.set(disableAudioRoutekey, true);
        NERtcEx.getInstance().setParameters(parameters);
        mDemoAudioManager = new DemoAudioManager(this, MODE_IN_COMMUNICATION, new AudioManagerEvents() {
            @Override
    ...
    private void initView() {
        ...
        mAudioRouteSp = findViewById(R.id.sp_audio_route);

        mBackIv.setOnClickListener(this);
        mAudioRouteBtn.setOnClickListener(this);
        mMuteMicBtn.setOnClickListener(this);
        mHangUpBtn.setOnClickListener(this);
        mAudioRouteSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String[] options = getResources().getStringArray(R.array.audio_route);
                switch (options[position]) {
                    case "default":
                        mDemoAudioManager.userSelectAudioDevice(AudioDevice.NONE);
                        break;
                    case "headset":
                        mDemoAudioManager.userSelectAudioDevice(AudioDevice.WIRED_HEADSET);
                        break;
                    case "bluetooth":
                        mDemoAudioManager.userSelectAudioDevice(AudioDevice.BLUETOOTH);
                        break;
                    case "speaker":
                        mDemoAudioManager.userSelectAudioDevice(AudioDevice.SPEAKER_PHONE);
                        break;
                }
            }
    ...
    private void changeAudioRoute() {
        ...
        if (mIsSpeakerPhone) {
            ...
            mDemoAudioManager.userSelectAudioDevice(AudioDevice.EARPIECE);
            mAudioRouteBtn.setText(getString(R.string.videocall_use_speaker));
        }
    }
